...
class SegNetLite(nn.Module):
    ...
    def __init__(self, kernel_sizes=[3, 3, 3, 3], down_filter_sizes=[32, 64, 128, 256],
        ...
        layers_pooling = [nn.MaxPool2d(kernel_size=pooling_kernel_sizes[i],
        ...
        layers_unpooling = [nn.MaxUnpool2d(kernel_size=pooling_kernel_sizes[i],
                                           stride=pooling_strides[i]) for i in range(self.num_up_layers)]
    ...
    def forward(self, x):
        ...
        for i in range(len(self.layers_conv_down)):
            ...
            x = self.relu(x)
            # Max pooling
            x, indicies = self.layers_pooling[i](x)
            pool_idx.append(indicies)

        # Decode layers
        for i in range(len(self.layers_conv_up)):
            # Max unpooling
            x = self.layers_unpooling[i](x, pool_idx[-i-1])
            # Convolution
