...
class SimlarityRegNet(nn.Module):
    def __init__(self, G):
        ...
        self.upconv2 = nn.ConvTranspose2d(in_channels=32, out_channels=8, kernel_size=3, stride=2, padding=1, output_padding=1)
        self.convout = nn.Conv2d(in_channels=16, out_channels=1,kernel_size=3, stride=1, padding=1)
        self.relu = nn.ReLU()
...
def warping(src_fea, src_proj, ref_proj, depth_values):
    ...
    with torch.no_grad():
        ...
        # TODO
        src_coords = torch.cat([x.unsqueeze(0),y.unsqueeze(0),torch.ones(1,H*W, dtype=torch.float32)], dim=0)  # [3,h*w]
    ...
    # TODO
    warped_src_fea = F.grid_sample(src_fea.unsqueeze(1).expand(B,D,C,H,W).reshape(B*D,C,H,W), warped_grid.reshape(B*D,H,W,-1)[...,:-1], mode="bilinear") # TODO: Volume/Space interpolation?
...
def group_wise_correlation(ref_fea, warped_src_fea, G):
    ...
    similarity = torch.sum(ref_fea_group.unsqueeze(3).expand(B,G,-1,D,H,W)*warped_src_fea_group, dim=2)/(C/G)  # [B,G,D,H,W]
    return similarity
...
def depth_regression(p, depth_values):
    ...
    # TODO
    B,D,H,W = p.size()
    depth_reg = torch.sum(depth_values.unsqueeze(-1).unsqueeze(-1).expand(B,D,H,W)*p, dim=1).unsqueeze(1)  # [B,1,H,W]
    return depth_reg
...
def mvs_loss(depth_est, depth_gt, mask):
    ...
    # TODO
    l1_loss = nn.L1Loss()
    input = depth_est*mask
    target = depth_est*mask
